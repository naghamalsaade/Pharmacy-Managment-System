        ...
        <div class="card">
            <div class="card-header d-flex justify-content-between bd-highlight mb-3">
                <div class="p-2 bd-highlight">
                   {{__('messages.medical_food_grid')}}
                </div>
                <div class="p-2 bd-highlight">
                    <a href="/medical-food/all-in-pharmacy"><button type="button" class="btn btn-list"><i
            ...
            <div class="card-body">
                ...
                <div class="row containerItems">
                    @foreach($medicalFoods as $medicalFood)
                        <div class="col-xl-3 col-lg-6 col-md-6 col-sm-6" data-search=<?php echo strtolower("{$medicalFood->name}"); ?>>
                            <div class="card text-center product">
                                ...
                                <div class="card-header d-flex justify-content-around bd-highlight">
                                    <div class="btn-delete-product p-2 bd-highlight">
                                        <a class="btn-delete" id="{{$medicalFood->id}}">
                                            <i class="fa fa-minus" aria-hidden="true"></i>
                                        </a>
                                    </div>
                                    <h3 class="p-2 bd-highlight name-product">{{$medicalFood->name}}</h3>
                                    <div class="id-product" style="display: none;">{{$medicalFood->id}}</div>
                                    <div class="price-product" style="display: none;">{{$medicalFood->selling_price}}</div>
                                    <div class="quantity-product" style="display: none;">{{$medicalFood->amount}}</div>
                                    <div class="expiry-product" style="display: none;">{{$medicalFood->date}}</div>
                                    <div class="price-product" style="display: none;">{{$medicalFood->selling_price}}</div>
                                    <div class="count-quantity" style="display: none;">0</div>
                                    <div class="btn-add-product p-2 bd-highlight">
                                        <a class="btn-add" id="{{$medicalFood -> id}}">
                                ...
                                <div class="card-body card-body-grid">
                                    <img src="{{asset('images/products/'.$medicalFood -> image)}}" class="d-block w-100 ">
                                    <label name="price" class="label-price">{{__('messages.price')}} : {{$medicalFood->selling_price}}
                                        S.P</label><br>
                                    <label name="Batch" class="label-batch"> {{__('messages.batch')}} : {{$medicalFood->id}}</label><br>
                                    <label> {{__('messages.quantity')}} </label>
                                    <input name="quantity" class="label-quantity" readonly="readonly" value="0">
                                </div>
                                <div class="card-footer">
                                    <a href="{{asset('medical-food/show-details/'. $medicalFood->id)}}"
